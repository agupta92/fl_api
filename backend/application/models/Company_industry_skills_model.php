<?php
    class Company_industry_skills_model extends CI_Model {
        ...
        public function get_assoc_skills($getCompany)
            {
                $sql= "Select skill_token as skill, count(*)  as freq from search_pro where stan_company='".urldecode($getCompany)."' group by 1 order by 2 DESC";
                $query = $this->db->query($sql);
				$count = $query->num_rows();				
                if($count==0)
                {
					...
					foreach ($query->result_array() as $row)
                        {
                           ...
                           $result['call']['response'][$countLooper]['freq']=$row['rel'];                           
                           $countLooper++;
                        }    
                }
				else
