<?php
    class Jd_model extends CI_Model {
		...
		public function get_assoc_company($getJD)
            {
                $sql= "Select stan_company as company, count(*) as freq from search_pro where JD='".urldecode($getJD)."' group by 1 order by 2 DESC LIMIT 20";			
                $query = $this->db->query($sql);
				$count = $query->num_rows();				
                if($count==0)
                {
		...
		public function get_available_salary($getJD)
            {
                $sql= "Select stan_company, JD, tent_sal as tent_sal from available_salaries where JD='".urldecode($getJD)."' group by 1,2 order by 3 DESC LIMIT 20";			
                $query = $this->db->query($sql);
				$count = $query->num_rows();				
                if($count==0)
                {
				...
				else if ($count>0)
                {
					...
					$result['call']['meta']['facets_maxSal']= str_replace(',', '', $maxSal) ;
					$result['call']['meta']['facets_minSal']= str_replace(',', '', $minSal) ;
					$result['call']['response']=$query->result();
                }
				else
				{
