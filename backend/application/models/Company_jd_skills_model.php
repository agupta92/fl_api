<?php
    class Company_jd_skills_model extends CI_Model {
		...
		if ($count == 0) {
            ...
            //var_dump($resultObj);
			//$resultObj->{'Higher Management'}=10;
			$totalCandid = intval($resultObj['Graduation']) + $resultObj['Higher Management'] + $resultObj['Higher Technical'] + $resultObj['Higher Research'];
            $tpPer = round(($resultObj['Total People']/$totalCandid)*100,2);
			$gradPer = round(($resultObj['Graduation']/$totalCandid)*100,2);
            $eduPer=100-$gradPer;
            $dpPer = round(($resultObj['Direct Placement']/$totalCandid)*100,2);
            $lPer = round(($resultObj['Laterals']/$totalCandid)*100,2);
            $lessThanOneYearPer = round(($resultObj['<1 year']/$totalCandid)*100,2);
            $oneToThreeYearPer = round(($resultObj['1-3 year']/$totalCandid)*100,2);
            $threeToFiveYearPer = round(($resultObj['3-5 year']/$totalCandid)*100,2);
            $fiveToSevenYearPer = round(($resultObj['5-7 year']/$totalCandid)*100,2);
            $greaterThanSevenYearPer = round(($resultObj['>7 year']/$totalCandid)*100,2);
            $hmPer=round(($resultObj['Higher Management']/$totalCandid)*100,2);
            $htPer = round(($resultObj['Higher Technical']/$totalCandid)*100,2);
            $hrPer = round(($resultObj['Higher Research']/$totalCandid)*100,2);
            
			$array[0][0] = array('key'=> 'Higher Education', 'value' => $eduPer,'subvalue'=>array(),'donut_text1'=>'Distributuion for the type of','donut_text2'=>'higher education of the people hired');
			$array[0][0]['subvalue'][0] = array('key'=> 'Technical', 'value' => $htPer,'color'=> "#9CCC65");
			$array[0][0]['subvalue'][1] = array('key'=> 'Research', 'value' => $hrPer,'color'=> "#558B2F");
		$array[0][0]['subvalue'][2] = array('key'=> 'Management', 'value' => $hmPer,'color'=> "#114D08");
			
			$array[0][1] = array('key'=> 'Graduation', 'value' => $gradPer,'subvalue'=>array(),'donut_text1'=>'Distributuion for the prior','donut_text2'=>'work experience of the people hired');
			$array[0][1]['subvalue'][0] = array('key'=> 'Direct Placement', 'value' => $htPer,'color'=> "#75000E ");
			$array[0][1]['subvalue'][1] = array('key'=> '<1 yr Exp', 'value' => $lessThanOneYearPer,'color'=> "#B11D00");
			$array[0][1]['subvalue'][2] = array('key'=> '1-3yr Exp', 'value' => $oneToThreeYearPer,'color'=> "#D12400");
			$array[0][1]['subvalue'][3] = array('key'=> '3-5yr Exp', 'value' => $threeToFiveYearPer,'color'=> "#E95337");
			$array[0][1]['subvalue'][4] = array('key'=> '5-7yr Exp', 'value' => $fiveToSevenYearPer,'color'=> "#F46F5A");
			$array[0][1]['subvalue'][5] = array('key'=> '>7yr Exp', 'value' => $greaterThanSevenYearPer,'color'=> "#F79A84");
			//$array[0]2] = array('key'=> 'Highered After Management Education', 'value' => $hmPer);
//var_dump($array);
			// $x = 0;
			// foreach ($query->result() as $key => $value) {
				// foreach ($value as $keys => $values) {
					// $finalArray[$x]['key'] = $keys;
					// $finalArray[$x]['value'] = $values;
					// $x++;
				// }
			// }
...
public function lateral_switch_vs_higher($getCompany, $getJd)
            {
...
public function get_Popular_Switches_FromCompany_forSpiderTool($getCompany, $getJd)
	            {
	                ...
	                //$sql= "Select r.company_1, r.JD_1, r.company_2, r.JD_2, r.tent_sal_2, r.freq, GROUP_CONCAT(v.`skill_token` SEPARATOR '|') as skills from (Select s.company_1, s.JD_1, s.tent_sal as tent_sal_1, s.company_2, s.JD_2, t.tent_sal as tent_sal_2 ,  s.freq from (Select m.company_1, m.JD_1, n.tent_sal, m.pro_s_no, m.company_2, m.JD_2, m.freq from (Select z.pro_s_no_1, z.company_1, z.JD_1, y.pro_s_no, y.stan_company as company_2, y.JD as JD_2, z.freq  from (Select p.old_bid, p.mega_mov_no as mega_mov_no_1, p.pro_s_no as pro_s_no_1,  p.stan_company as company_1, p.JD as JD_1, (p.mega_mov_no+1) as mega_mov_no_2, q.pro_s_no as pro_s_no_2, count(q.old_bid) as freq from   (Select old_bid, mega_mov_no, pro_s_no,stan_company, JD, duration_with_2015 as stay from edu_pro where stan_company='".urldecode($getCompany)."' group by 1,2) p, edu_pro q   where p.old_bid=q.old_bid and q.mega_mov_no=(p.mega_mov_no+1) and (p.pro_s_no is not null and q.pro_s_no is not null) and p.pro_s_no <> q.pro_s_no group by 1,3,7) z,  search_pro y where z.pro_s_no_2=y.pro_s_no group by 1,4 order by 5 DESC) m  LEFT JOIN available_salaries n on m.pro_s_no_1=n.pro_s_no) s LEFT JOIN available_salaries t on s.pro_s_no=t.pro_s_no ORDER BY 7 DESC) r LEFT OUTER JOIN search_pro v ON r.company_2=v.stan_company and r.JD_2=v.JD group by 1,2,3,4";
	             	$sql = "Select skill_token as skill, count(*)  as count from search_pro r,  (Select z.pro_s_no_1, z.company_1, z.JD_1, y.pro_s_no, y.stan_company as company_2, y.JD as JD_2, z.freq  from (Select p.old_bid, p.mega_mov_no as mega_mov_no_1, p.pro_s_no as pro_s_no_1,  p.stan_company as company_1, p.JD as JD_1, (p.mega_mov_no+1) as mega_mov_no_2, q.pro_s_no as pro_s_no_2, count(q.old_bid) as freq from   (Select old_bid, mega_mov_no, pro_s_no,stan_company, JD, duration_with_2015 as stay from edu_pro where stan_company='".urldecode($getCompany)."' and JD='".urldecode($getJd)."' group by 1,2) p,  edu_pro q   where p.old_bid=q.old_bid and q.mega_mov_no=(p.mega_mov_no+1) and (p.pro_s_no is not null and q.pro_s_no is not null) and p.pro_s_no <> q.pro_s_no group by 1,3,7) z,  search_pro y where z.pro_s_no_2=y.pro_s_no group by 1,4 order by 5 DESC) m  where r.pro_s_no=m.pro_s_no group by 1 order by 2 DESC";
	                $skillsArray=array();
	                $skillString='';
					$jd_1='';
	                $query = $this->db->query($sql);
				...
				else if ($count>0)
                {
					 ...
					 $result['call']=array('success'=>true,'code'=>0,'message'=>'null');
					// $result['call']['meta']=array('total'=>$count);
					// $result['call']['response']=$query->result();$result['call']=array('success'=>true,'code'=>0,'message'=>'success');
                    $result['call']['meta']=array('total'=>$count);
					$result['call']['response']=$query->result();
					//var_dump($result);
					//$result['call']['response']= array('skills'=>explode('|', $skillString));
                        $x=0;
						// foreach($query->result_array() as $row){
                            // $skillString.=$row['skills'];
							// $jd_1.=$row['JD_1'].'|';
							// $result['call']['response'][$x]->skills=explode('|',$row['skills']);
							// $x++;
                         // }
                                        // $uniqueskills =array_unique(explode('|', $skillString));
										// $uniqueJd1= array_unique(explode('|', $jd_1));
// //                                        array_push($skillsArray,$row->skills);
                                        // //array_unique($skillsArray);
                                        // $result['call']['meta']['facets_skills']=array_values($uniqueskills);
										// $uniqueJd1=array_values($uniqueJd1);
										// if(empty($uniqueJd1[count($uniqueJd1)-1])) {
											// unset($uniqueJd1[count($uniqueJd1)-1]);
										// }
										//$result['call']['meta']['facets_jobs']=$uniqueJd1;
                }
				else
				{
					$result['call']=array('success'=>false,'code'=>'2','message'=>'something went wrong');
