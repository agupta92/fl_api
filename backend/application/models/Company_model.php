<?php
    class Company_model extends CI_Model {
		...
		public function get_assoc_jd($getCompany)
            {
                ...
                $sql= "Select JD, count(*) as freq from search_pro where stan_company='".urldecode($getCompany)."' group by 1 order by 2 DESC";
		...
		public function get_jd_path($getCompany)
            {
		...
		public function get_available_salaries($getCompany)
            {
                ...
                $sql= "Select a.JD,FORMAT( b.tent_sal,0) as tent_sal, a.total_people from  (Select pro_s_no, JD, total_people from search_pro where stan_company='".urldecode($getCompany)."' group by 1) a, available_salaries b where a.pro_s_no=b.pro_s_no group by 1 order by 3 DESC";
		...
		public function get_Popular_Switches_FromCompany($getCompany)
			...
			public function get_Popular_Switches_FromCompany_forSpiderTool($getCompany)
	            {
                    ...
                    foreach($arrayMain as $obj){
                        foreach($obj as $key=>$value) {
                            ...
                            foreach($value as $keys=>$values) {
								...
								$newstr = substr_replace($keys, "<br>", strlen ($keys)-5, 0);
