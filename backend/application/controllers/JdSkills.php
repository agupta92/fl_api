<?php
    class JdSkills extends CI_Controller {
		
        public function __construct()
        {
            parent::__construct();
            $this->load->model('Jd_skill_model','',TRUE);
        ...
        public function getAssocIndustries($jd){
			...
			if($result) {					
				...
				$data['query'] = $this->Jd_skill_model->get_assoc_industries($jd);
        ...
        public function getAssocSkills($jd){
			...
			if($result) {					
				...
				$data['query'] = $this->Jd_skill_model->get_assoc_skills($jd);
        ...
        public function getAssocCompanies($jd){
			...
			if($result) {					
				...
				$data['query'] = $this->Jd_skill_model->get_assoc_company($jd);
		...
		public function getAssocSal($jd){
			...
			if($result) {					
				...
				$data['query'] = $this->Jd_skill_model->get_available_salary($jd);
